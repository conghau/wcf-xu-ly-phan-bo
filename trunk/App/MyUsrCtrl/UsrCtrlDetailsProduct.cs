...
namespace App.MyUsrCtrl
{
    public partial class UsrCtrlDetailsProduct : MyUserControl
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    ...
                    Utilities.ShowMessage("Lưu vào CSDL thành công");
                    IsRemove = true;
                }
                else
                {
                    Utilities.ShowMessageError("Có lỗi khi lưu");
                    _productRepository.Refresh();
                }
            }
            catch (SqlException)
            {
                Utilities.ShowMessageError("Không thể thêm sản phẩm đã có trong CSDL .");
                _productRepository.Refresh();
            }
        }
        ...
        private void CopyImage()
        {
            ...
            {
                if (Utilities.ShowQuestionMessage(string.Format("File {0} đã có, Bạn có muốn xóa ?", txtImage.Text))
                == DialogResult.Yes)
            ...
            catch (Exception ex)
            {
                ...
                Utilities.ShowMessageError("Lỗi khi copy");
            }
        }
